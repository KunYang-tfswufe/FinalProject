...
12V 3A 直流电源适配器（给水泵/植物灯等用，和树莓派电源分离）
执行器与驱动（可控水泵/风扇/灯）
12V 迷你潜水泵 或 12V 蠕动泵（演示更稳推荐“蠕动泵”，流量可控、无回流）
3.3V 兼容的 MOSFET 驱动模块 2 个（如“逻辑电平 N‑MOSFET 模块”，用于泵/风扇；优先 MOSFET 而非只能 5V 触发的继电器）
续流二极管若用电机/继电器（1N5819/1N4007）
12V 全光谱植物生长灯（LED 灯条+铝槽，演示光照控制）
传感器（精度和耐用性补齐）
BH1750 光照传感器（I2C，3.3V）
电容式土壤湿度传感器（3.3–5V，耐腐蚀，替代你现有的电阻式探针）
防水 DS18B20（测土壤/水温；补足 DHT11 的精度和耐用性不足）
（备选升级）SHT31‑D 或 BME280（温湿度更稳，后期可替换 DHT11）
水路与机电辅材
...
12V/5V 小风扇（空气流通控制）
DC‑DC 降压模块（如 LM2596，用 12V 总线分出稳压 5V 给执行器，仍与树莓派电源分离供电但共地）
演示物品/作物（建议按季节与速度选择）
首选（贴合课题）
藏红花球茎（季节性：每年 8–10 月上架，非季节时难买，备选下列植物）
花盆（带排水孔）+ 育苗盘 + 通用营养土/泥炭土 + 珍珠岩
替代快生长作物（1–2 周内可见变化，便于答辩演示）
小麦草/绿豆/红豆（发芽快，自动浇水效果直观）
罗勒/薄荷/小葱（耐活、对光照/水分响应明显）
水路演示替代物
海绵/陶粒模拟“土壤”，更容易定量展示湿度变化
与你现有硬件的适配说明
你已有的“5V 继电器模块（低电平触发）”不一定能被 STM32 的 3.3V 稳定驱动，建议增购“3.3V 兼容 MOSFET 驱动模块”作为通用执行器驱动（泵/风扇/灯都可用）。
你已有的“电阻式土壤湿度传感器”易腐蚀、读数漂移大；演示可先用，但正式建议换“电容式”。
DHT11 可继续用来打通链路，后续用 SHT31‑D/BME280 替换以提升精度与论文说服力。
最小成套清单（一天内能跑通的闭环）
5V 3A 树莓派电源、microSD 32GB
12V 3A 适配器、12V 蠕动泵、硅胶管+单向阀+水箱
MOSFET 驱动模块 x2、续流二极管
BH1750、 电容式土壤湿度、DS18B20
花盆+营养土（或育苗盘）+ 快速发芽的豆类/小麦草
如需，我可以按你本地电商平台给出具体商品链接与参数（接口口径、尺寸、电流冗余）及接线图。
关键点
采购补齐：12V 执行器链路（泵/灯）、3.3V 兼容驱动、精度更高的光照/土壤/温度传感器。
演示物品：藏红花球茎（当季）或快速发芽作物；盆+土+水路耗材。
安全/可靠：强弱电与水路隔离，执行器与树莓派分离供电但共地，带续流保护。