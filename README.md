...
- 电源
  - 5V 3A 树莓派电源：Raspberry Pi 官方 5V 3A USB‑C 电源（或等效）
  - 5V 执行器电源：5V 2A–3A DC 适配器（圆头 5.5×2.1 mm 或 USB 口）

- 执行器与驱动
  - 5V 潜水泵：USB 5V 迷你潜水泵（80–120 L/h，扬程 0.6–1 m）
  - 5V 蠕动泵：微型 5V 蠕动泵（60–100 mL/min，配 4×6 mm 硅胶管）
  - MOSFET 模块：单路逻辑电平 N‑MOSFET 模块（IRLZ44N/IRLZ34N/AO3400A 等，3.3V 触发）
  - 续流二极管：1N5819 或 SS14（并联在电机两端）

- 传感器
  - 光照：GY‑302 BH1750 I2C 模块（3.3V）
  - 土壤湿度：电容式土壤湿度传感器 V1.2（防腐蚀，3.3–5V）
  - 温度：防水 DS18B20 探头（3 线制，1 m 线）

- 补光与通风
  - 5V 植物灯：USB 5V 植物补光灯条（0.5–1 m，带开关）
  - 风扇：5V 4010/5015 直流风扇（两线）

- 水路辅材
  - 软管：硅胶软管 4×6 mm（按泵接口选）
  - 单向阀：鱼缸空气泵单向阀（约 5 mm 接口）
  - 水箱：5–10 L 带盖塑料箱

- 演示作物（低预算）
  - 观赏辣椒小盆栽 / 长寿花 / 一串红（红色系现成盆栽，直观接近藏红花效果）
